 ...
 */
export default function PolymerRedux<T>(store: Store) {
    if (!store) {
    ...
    const bind = (element, properties) => {
        ...
        const update = state => {
            ...
            bindings.forEach(name => {
                ...
                const changed = element._setPendingPropertyOrPath(
                    name,
                    value,
                    true
                )
     ...
     */
    return (parent: { new (): PolymerElement }) =>
        class ReduxMixin extends parent {
            private _reduxActions
